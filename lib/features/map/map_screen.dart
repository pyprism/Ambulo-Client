...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: history.when(
        data: (points) {
          // Show the map (with places/friends overlays) as soon as there's
          // *anything* to put on it — not just once this device has its own
          // history. A brand-new device with a friend already sharing, or
          // one that's only ever added places, shouldn't see an empty state.
          if (points.isEmpty && places.isEmpty && friendLocations.isEmpty) {
          ...
          final center = points.isNotEmpty
              ? latlong.LatLng(points.first.latitude, points.first.longitude)
              : friendLocations.isNotEmpty
              ? latlong.LatLng(
                  friendLocations.first.latitude,
                  friendLocations.first.longitude,
                )
              : latlong.LatLng(places.first.latitude, places.first.longitude);
          ...
          // The app-wide SelectionArea (for copyable text) otherwise fights
          // flutter_map's own drag-to-pan/pinch-to-zoom gestures.
