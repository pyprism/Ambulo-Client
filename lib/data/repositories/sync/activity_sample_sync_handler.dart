...
class ActivitySampleSyncHandler implements SyncTypeHandler {
  ActivitySampleSyncHandler(this._db);

  final AppDatabase _db;

  @override
  String get typeName => 'activity_sample';

  Map<String, dynamic> _toWire(ActivitySample row) {
    return {
      'id': row.id,
  ...
  Future<void> applyUploadResult({
    ...
    for (final id in conflicts) {
      await (_db.update(
        ...
        const ActivitySamplesCompanion(syncState: Value(SyncState.conflict)),
      );
    }
    for (final entry in rejected) {
      ...
      await (_db.update(
        ...
        const ActivitySamplesCompanion(syncState: Value(SyncState.failed)),
      );
    }
  }
  ...
  Future<void> markSynced(String id) async {
    await (_db.update(
      ...
      const ActivitySamplesCompanion(syncState: Value(SyncState.synced)),
    );
  }
  ...
  Future<void> _adoptServerRevOnly(
    String id,
    ActivitySample existing,
    Map<String, dynamic> json,
  ) async {
    final serverRev = json['server_rev'] as int?;
    if (serverRev == null || serverRev == existing.serverRev) return;
    await (_db.update(_db.activitySamples)..where((t) => t.id.equals(id)))
        .write(ActivitySamplesCompanion(serverRev: Value(serverRev)));
  }

  @override
  Future<SyncTypeCounts> counts() async {
    final t = _db.activitySamples;
